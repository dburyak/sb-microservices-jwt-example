...
import static com.dburyak.example.jwt.otp.repository.migration.M_0001_CreateOTPsCollection.COLLECTION_OTPS;
import static java.util.concurrent.TimeUnit.SECONDS;

@ChangeUnit(id = "0002-create-otps-indexes", order = "0002", author = "dmytro.buryak")
public class M_0002_CreateOTPsIndexes {
    static final String IDX_USER_UUID_1_TYPE_1_TENANT_UUID_1 = "userUuid_1_deviceId_1_type_1_tenantUuid_1";
    ...
    public void executeBeforeWithoutTx(com.mongodb.client.MongoDatabase mongo) {
        var collection = mongo.getCollection(COLLECTION_OTPS);
        collection.createIndex(new Document(Map.of(
                        FIELD_USER_UUID, 1,
                        FIELD_DEVICE_ID, 1,
                        FIELD_TYPE, 1,
                        FIELD_TENANT_UUID, 1)),
                new IndexOptions().name(IDX_USER_UUID_1_TYPE_1_TENANT_UUID_1).unique(true));
        collection.createIndex(new Document(Map.of(
                        FIELD_EXPIRES_AT, 1)),
    ...
    public void rollbackBeforeExecution(com.mongodb.client.MongoDatabase mongo) {
        var collection = mongo.getCollection(COLLECTION_OTPS);
        collection.dropIndex(IDX_USER_UUID_1_TYPE_1_TENANT_UUID_1);
