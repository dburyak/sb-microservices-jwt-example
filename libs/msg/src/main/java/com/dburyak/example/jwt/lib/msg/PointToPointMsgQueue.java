...
public interface PointToPointMsgQueue {
    ...
    <T> void subscribe(String topic, String consumerGroup, Predicate<AppAuthentication> access,
            Consumer<Msg<T>> handler);

    default <T> void subscribe(String topic, Predicate<AppAuthentication> access, Consumer<Msg<T>> handler) {
